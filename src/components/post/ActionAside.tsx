...
const shareList = [
  {
    ...
    onClick: (data: ShareData) => {
      window.open(
        `https://service.weibo.com/share/share.php?type=3&url=${encodeURIComponent(data.url)}&title=${data.text ? encodeURIComponent(data.text) : encodeURIComponent(site.title)}`,
      )
    },
  },
  {
    ...
    onClick: (data: ShareData) => {
      window.open(
        `https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=${encodeURIComponent(data.url)}&desc=${encodeURIComponent(data.text)}&title=${encodeURIComponent(site.title)}`,
      )
    },
  },
  {
...
function ShareModal({ url, text }: { url: string; text: string }) {
  return (
    ...
    >
      <h2 className="px-3 py-1 font-bold">扫码阅读</h2>
...
function DonateContent() {
  return (
    <motion.div
      ...
      exit={{ y: 20, opacity: 0 }}
    >
      <h2 className="text-center mb-5">坚持原创技术分享，您的支持将鼓励我继续创作！</h2>
